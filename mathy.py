...
def name_reps(name):
    ...
    if response is not None: 
        html = BeautifulSoup(response, 'html.parser') 
        hit_link = [a for a in html.select('a') if a ['href'].find(latest-60) >= 0]
        ...
        if len(hit_link) > 0: 
            ...
            try: 
                ...
                log_error("JuicySoup Error: could not parse {} as int value".format(link_text))
            
    log_error("JuicySoup Error: No pageviews found for {}".format(name))
    return None
